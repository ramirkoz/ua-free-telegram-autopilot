...
from .ukrainian_quality import apply_safe_ukrainian_fixes, final_language_blockers, language_quality_issues, human_style_issues, needs_human_copyedit, remove_unattributed_editorial_sentences, remove_source_author_meta_sentences
from .models import Channel, Decision
from .rewrite_verifier import assess_rewrite, build_revision_feedback, hard_editorial_blockers


LOG = logging.getLogger("telegram_autopilot.production")
...
POST_FORMAT_PREFIX = "telegram-post-v18:"
...
def _compact_source(article: Row, *, local: bool, hard_limit: int) -> str:
    """Compatibility wrapper returning the deterministic Evidence Pack."""
...
- avoid unsupported editorial verdicts such as «один з найкращих», «еталонне рішення», «ідеально підходить»;
- NEVER mention the article's author, reviewer, editor or journalist as a person in the post, and never write their expectations/opinions (for example «автор матеріалу очікує...»). Keep an objective verified fact if SOURCE supports it; otherwise omit the commentary. This does not forbid naming a person who is actually part of the news event or an official quoted source;
    ...
    for _ in range(max(1, int(max_candidates))):
        ...
        try:
            ...
            if _soft_qa_failure(exc) and qa_attempts_by_model[model_key] == 1:
                ...
                # it. Older routing skipped the model immediately, which was wasteful when
...
def decide(channel: Channel, article: Row, recent: list[Row], *, hard_limit: int = MEDIA_POST_HARD_LIMIT, format_marker: str | None = None) -> Decision:
    ...
    def validator(raw: str) -> dict[str, str]:
        ...
        normalized_body = remove_source_author_meta_sentences(normalized_body)
    ...
    # Production writing now starts with the reviewed static allow-list. Codex is
    # priority one when the ChatGPT session is healthy; Gemini is the second trusted
    # cloud editor. Other reviewed providers and local AI remain useful generators,
    # but their drafts may never autopublish without a trusted final editorial pass.
    result, rewrite = _route_with_post_qa(
        ...
        local_timeout_seconds=50,
        task_timeout_seconds=105,
        max_candidates=6,
    )
    body = rewrite["body"]
    selected_result = result
    quality = assess_rewrite(body, hard_limit=hard_limit)
    draft_blockers = hard_editorial_blockers(body)
    draft_language = language_quality_issues(body)
    draft_style = human_style_issues(body)
    trusted_providers = {"codex", "gemini"}
    generator_trusted = str(result.provider or "").casefold() in trusted_providers
    LOG.info(
        "Rewrite draft provider=%s model=%s score=%s trusted=%s hard_blockers=%d language_warnings=%d style_warnings=%d",
        result.provider, result.model, quality.score, generator_trusted, len(draft_blockers), len(draft_language), len(draft_style),
    )

    # A clean high-quality Codex/Gemini draft stays one-call fast. Every fallback
    # draft (NVIDIA/Groq/Cloudflare/local), and every weak or structurally suspicious
    # trusted draft, is rewritten/verified once more from SOURCE evidence by a
    # trusted editor. This is deliberately fail-closed: an unattended publisher
    # must prefer retrying a story over emitting syntactically broken nonsense.
    needs_trusted_editor = (
        not generator_trusted
        or quality.needs_second_candidate
        or bool(draft_blockers)
        or bool(draft_language)
        or bool(draft_style)
        or needs_grammar_polish(body)
        or needs_human_copyedit(body)
    )
    if needs_trusted_editor:
        feedback_items = list(draft_blockers) + list(quality.issues) + list(draft_language) + list(draft_style)
        feedback = "; ".join(list(dict.fromkeys(str(item) for item in feedback_items if str(item).strip()))[:8])
        trusted_prompt = (
            cloud_prompt
            + "\n\nFINAL TRUSTED EDITOR PASS. You are the last editor before unattended Telegram publication. "
              "Rebuild the post from SOURCE EVIDENCE PACK, using the previous draft only as a warning/example. "
              "Do not preserve its wording if it is awkward. Use natural standard Ukrainian, concise human newsroom syntax, "
              "no Russian-only letters, no mixed-alphabet words, no looping phrases, no invented words and no repeated sentences. "
              "Preserve every factual relation, attribution, uncertainty, number and year; never add a fact. "
              f"Keep the final body within {hard_limit} characters and return ONLY the body."
            + ("\nProblems detected in previous draft: " + feedback if feedback else "")
            + "\n\nPREVIOUS DRAFT:\n" + body[: min(len(body), hard_limit)]
        )

        def trusted_validator(raw: str) -> dict[str, str]:
            checked = validator(raw)
            candidate = checked["body"]
            blockers = hard_editorial_blockers(candidate)
            if blockers:
                raise ProductionPipelineError("Trusted editorial blocker: " + "; ".join(blockers))
            candidate_quality = assess_rewrite(candidate, hard_limit=hard_limit)
            if not candidate_quality.publishable:
                raise ProductionPipelineError(
                    f"Trusted editorial quality {candidate_quality.score}/100: "
                    + "; ".join(candidate_quality.issues[:5])
                )
            return checked

        try:
            trusted_result, trusted = _route_with_post_qa(
                trusted_prompt,
                trusted_prompt,
                trusted_validator,
                max_output_tokens=380 if media_mode else 1000,
                local_max_output_tokens=380 if media_mode else 1000,
                cloud_timeout_seconds=24,
                local_timeout_seconds=12,
                task_timeout_seconds=75,
                allowed_providers=trusted_providers,
                max_candidates=3,
            )
        except (AIRouterError, PostAIQAExhausted) as exc:
            LOG.warning(
                "Trusted editor unavailable/rejected generator=%s/%s error=%s",
                result.provider, result.model, exc,
            )
            provider_outage = isinstance(exc, AIRouterError) or bool(getattr(exc, "provider_outage", False))
            raise PostAIQAExhausted(
                "Trusted editor (Codex/Gemini) не підтвердив безпечний фінальний текст. " + str(exc),
                tuple(getattr(exc, "failures", ()) or (str(exc),)),
                provider_outage=provider_outage,
            ) from exc
        body = trusted["body"]
        selected_result = trusted_result
        quality = assess_rewrite(body, hard_limit=hard_limit)
        LOG.info(
            "Trusted editor success provider=%s model=%s score=%s generator=%s/%s",
            trusted_result.provider, trusted_result.model, quality.score, result.provider, result.model,
        )

    # Final publication language gate. LanguageTool is a preferred local proofreader,
    # not a liveness dependency. If its Windows server is unavailable, the text
    # continues unchanged into the deterministic Ukrainian blocker set below.
    # Any LanguageTool edit that is available is still fully revalidated against
    # facts, numbers and years before publication.
    ...
    if not looks_ukrainian(body):
        raise ProductionPipelineError("Фінальний український gate: текст не визначено як природну українську прозу.")

    hard_blockers = hard_editorial_blockers(body)
    if hard_blockers:
        raise ProductionPipelineError("Фінальний editorial blocker: " + "; ".join(hard_blockers))
    if not quality.publishable:
        raise ProductionPipelineError(
            f"Фінальний editorial quality {quality.score}/100 нижче безпечного порога 82/100: "
            + "; ".join(quality.issues[:5])
        )
    LOG.info(
        "Final publish gate provider=%s model=%s score=%s lt_changes=%s chars=%s",
        selected_result.provider, selected_result.model, quality.score, lt_result.changes, len(body),
    )
    ...
    return Decision(
        ...
        reason=(f"Матеріал пройшов Evidence Pack, Fact Guard, trusted-editor/final-UA gate та editorial quality ({quality.score}/100). {lt_note}" + (" Language warnings: " + "; ".join(language_quality_issues(body)) if language_quality_issues(body) else "") + (" Style warnings: " + "; ".join(human_style_issues(body)) if human_style_issues(body) else "")),
