...
_HARD_LANGUAGE_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"\bвечной\b", re.I), "русизм «вечной»"),
    (re.compile(r"\bлекарств(?:о|а|у|ом|і|ами|ах)?\b", re.I), "русизм «лекарств…»"),
    (re.compile(r"\bопередно\b", re.I), "ненормативна калька «опередно»"),
    (re.compile(r"\bпродвиг\w*", re.I), "русизм «продвиг…»"),
    (re.compile(r"\bвіровит\w*", re.I), "ненормативна форма «віровит…»"),
    (re.compile(r"\bвідреагував\s+інше\b", re.I), "порушена конструкція «відреагував інше»"),
    (re.compile(r"\bзмінит(?:ися|ись)\s+до\s+невизначення\b", re.I), "калька «змінитися до невизначення»"),
    (re.compile(r"\bкомерційне\s+впливання\b", re.I), "неприродна конструкція «комерційне впливання»"),
)
