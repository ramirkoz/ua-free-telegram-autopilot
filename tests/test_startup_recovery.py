...
def test_startup_recovery_matches_rc32_contract(tmp_path: Path, monkeypatch):
    db = Database(tmp_path / 'recovery.sqlite3')
    _seed(db)
    monkeypatch.setattr(startup_recovery, 'clear_router_cooldowns', lambda: None)

    result = startup_recovery.recover_interrupted_work(db)

    assert result == {
        'processing_to_retry': 1,
        'external_to_unknown': 1,
        'runtime_health_reset': 1,
        'recent_technical_requeued': 0,
    }
    assert db.get_article(1)['status'] == 'retry'
    assert db.get_article(1)['next_retry_at'] is None
    assert db.get_article(2)['status'] == 'telegraph_writing'
    assert db.get_article(3)['status'] == 'unknown'
    assert [row['id'] for row in db.pending_articles(1, limit=10)] == [1]

    second = startup_recovery.recover_interrupted_work(db)
    assert second == {
        'processing_to_retry': 0,
        'external_to_unknown': 0,
        'runtime_health_reset': 0,
        'recent_technical_requeued': 0,
    }
