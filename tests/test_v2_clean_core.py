...
from telegram_autopilot.v2.domain import BlockedBy, ChannelPolicy, Decision
    ...
    with store.connect() as con:
        ...
        con.execute(
            """INSERT INTO channel_policies(channel_id,enabled,purpose,audience,selection_rules,rejection_rules,writing_rules,style_rules,
               ...
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                channel_id, 1, p.purpose, p.audience, p.selection_rules, p.rejection_rules,
                p.writing_rules, p.style_rules, p.positive_examples, p.negative_examples,
                p.extra_instructions, p.selector_extra_prompt, p.writer_extra_prompt,
                p.media_policy, p.target_min_chars, p.target_max_chars, stamp,
            ),
        )
...
def seed_source(store: V2Store, channel_id: int, source_id: int, *, url: str | None = None) -> None:
    with store.connect() as con:
        con.execute(
            ...
            (source_id, channel_id, f"S{source_id}", url or f"https://source{source_id}.example/feed"),
        )
...
def test_source_is_hard_publication_gate(tmp_path: Path):
    ...
    seed_channel(store, 1)
    seed_source(store, 1, 1)
...
def test_fresh_first_and_per_channel_claim(tmp_path: Path):
    store = V2Store(tmp_path / "v2.sqlite3")
    for cid in (1, 2):
        seed_channel(store, cid)
        seed_source(store, cid, cid)
    ...
    old_stamp = (now - timedelta(hours=8)).isoformat(timespec="seconds")
    fresh_stamp = (now - timedelta(hours=1)).isoformat(timespec="seconds")
    with store.connect() as con:
        con.execute("UPDATE articles SET source_published_at=? WHERE id=?", (old_stamp, old))
        con.execute("UPDATE articles SET source_published_at=? WHERE id=?", (fresh_stamp, fresh))
...
def test_provider_success_wakes_waiting_ai(tmp_path: Path):
    ...
    job = store.claim_job(channel_id=1, worker_id="w")
    store.defer_job(
        int(job["id"]),
        blocked_by=BlockedBy.AI,
        error_code="WAITING_AI",
        detail="provider outage",
        retry_seconds=3600,
        count_attempt=False,
    )
    gateway = AIGateway(store)
    gateway._mark_success("codex", "account-default", "account-default")
    with store.connect() as con:
        row = con.execute(
            "SELECT j.state,a.blocked_by,a.retry_count FROM jobs j JOIN articles a ON a.id=j.article_id WHERE a.id=?",
            (aid,),
        ).fetchone()
...
def test_strict_dedupe_same_source_different_rows_do_not_merge(tmp_path: Path):
    ...
    seed_source(store, 1, 1)
    add_article(
        store, 1, 1, "1", title="NVIDIA releases DLSS 4.0 update",
        url="https://s/1", body="NVIDIA DLSS 4.0 new game update A",
    )
    b = add_article(
        store, 1, 1, "2", title="NVIDIA releases DLSS 4.1 update",
        url="https://s/2", body="NVIDIA DLSS 4.1 another game update B",
    )
...
def test_canonical_url_is_a_hard_identity_guard(tmp_path: Path):
    store = V2Store(tmp_path / "v2.sqlite3")
    seed_channel(store, 1)
    seed_source(store, 1, 1)
    seed_source(store, 1, 2)
...
def test_monitoring_never_fail_opens_on_ai_outage(tmp_path: Path):
    ...
    p = ChannelPolicy(
        channel_id=3,
        selection_rules="лише офіційні рішення громад",
        rejection_rules="не брати привітання",
    )
    seed_channel(store, 3, mode="monitoring", policy=p)
    seed_source(store, 3, 3)
    aid = add_article(
        store, 3, 3, "1", title="Рішення громади",
        url="https://t.me/source/1", body="Офіційне рішення",
    )

    class DeadGateway:
        def run(self, *args, **kwargs):
            raise GatewayExhausted("AI down", retry_seconds=300, provider_outage=True)

    engine = EditorialEngine(store, DeadGateway())
    with pytest.raises(GatewayExhausted):
        engine.select(store.get_channel(3), store.get_article(aid))
    assert store.get_article(aid)["decision"] == "PENDING"


def make_legacy_db(path: Path) -> None:
    now = datetime.now(timezone.utc)
    published_source = (now - timedelta(hours=6)).isoformat(timespec="seconds")
    published_at = (now - timedelta(hours=5)).isoformat(timespec="seconds")
    recent_at = (now - timedelta(hours=2)).isoformat(timespec="seconds")
    old_at = (now - timedelta(days=10)).isoformat(timespec="seconds")
    created_at = (now - timedelta(days=30)).isoformat(timespec="seconds")
    ...
    """)
    con.execute(
        "INSERT INTO channels VALUES(1,'CTRL+UA','@ctrl','tech',1,1,5,11,72,48,3,'[{\"name\":\"AI\",\"weight\":70}]','en_to_uk',1,1,'00:00','23:59',1,1,4,6,'editorial','auto',1,450,'preserve-me',?,?)",
        (created_at, updated_at),
    )
    con.execute(
        "INSERT INTO channel_policies VALUES(1,1,'tech purpose','UA','include research','exclude promo','write concise','human','good','bad','extra','selector extra','writer extra','preferred',280,780,?)",
        (updated_at,),
    )
    con.execute("INSERT INTO sources VALUES(1,1,'rss','Src','https://src/feed',1,1,'','','50','keep-source')")
    con.execute(
        """INSERT INTO articles(id,channel_id,source_id,external_id,title,url,normalized_url,raw_text,content_hash,source_published_at,discovered_at,status,published_at,telegram_message_id,teaser_text,media_json,article_layout_json)
           VALUES(10,1,1,'p','Published','https://src/p','https://src/p','facts','h1',?,?,'published',?,'555','published text','[]','{}')""",
        (published_source, published_source, published_at),
    )
    con.execute(
        """INSERT INTO articles(id,channel_id,source_id,external_id,title,url,normalized_url,raw_text,content_hash,source_published_at,discovered_at,status,last_error,retry_count,next_retry_at)
           VALUES(11,1,1,'r','Recent','https://src/r','https://src/r','facts','h2',?,?,'error','temporary',9,'2099-01-01T00:00:00+00:00')""",
        (recent_at, recent_at),
    )
    con.execute(
        """INSERT INTO articles(id,channel_id,source_id,external_id,title,url,normalized_url,raw_text,content_hash,source_published_at,discovered_at,status)
           VALUES(12,1,1,'o','Old','https://src/o','https://src/o','facts','h3',?,?,'new')""",
        (old_at, old_at),
    )
    con.execute(
        "INSERT INTO article_feedback VALUES(10,1,'555',?,?,100,4,1,5,0,2,0)",
...
def test_legacy_migration_preserves_editorial_settings_not_runtime_state(tmp_path: Path):
    ...
    target = V2Store(tmp_path / "v2.sqlite3")
    ...
    assert extras["custom_future_setting"] == "preserve-me"
    assert source_extras["unknown_source_setting"] == "keep-source"
...
def test_runtime_channel_isolation_on_ai_outage(tmp_path: Path):
    ...
    for cid in (1, 2, 3):
        seed_channel(store, cid)
        seed_source(store, cid, cid)
        add_article(store, cid, cid, f"a{cid}", title=f"Article {cid}", url=f"https://s{cid}/a")
    runtime = RuntimeEngine(store)

    class FakeEditorial:
        def process_article(self, aid, heartbeat=None):
            if heartbeat:
                heartbeat()
            row = store.get_article(aid)
            if int(row["channel_id"]) == 1:
                raise GatewayExhausted("provider down", retry_seconds=300, provider_outage=True)
    ...
    class FakePublisher:
        def publish_ready(self, cid, heartbeat=None):
            if heartbeat:
                heartbeat()
            return 0
    ...
    try:
        ...
        while time.time() < deadline:
            with store.connect() as con:
                c1 = con.execute(
                    "SELECT a.blocked_by,j.state,a.retry_count FROM articles a JOIN jobs j ON j.article_id=a.id WHERE a.channel_id=1"
                ).fetchone()
